...
export default defineComponent({
    ...
    setup(props, { slots }: SetupContext) {
        ...
        const renderColgroup = () => {
            return tableWidth.value ? (
                <colgroup class="ivy-table-row is-title">
                    {tableData.value.map(c => (c.width ? <col width={parseFloat(c.width)}></col> : <col />))}
                </colgroup>
            ) : (
                []
            );
        ...
        return () => (
            <div class={["ivy-table", { "is-border": props.border }]} ref={tableRef} style={{ width: `${tableWidth.value}px` }}>
