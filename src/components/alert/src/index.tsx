...
export default defineComponent({
    ...
    setup(props, { slots }) {
        ...
        const renderIcon = () => {
            return h(resolveComponent("ivy-icon"), { name: iconName[props.type], class: "ivy-alert__icon" });
        };
        ...
        return () => {
            if (props.showIcon) {
                return (
                    <div class={classStr}>
                        {() => [renderIcon(), <div class="ivy-alert__content">{() => [renderTitle(), renderDesc()]}</div>]}
                    </div>
                );
            } else {
                return <div class={classStr}>{() => [renderTitle(), renderDesc()]}</div>;
            }
        };
    },
});
